...
/**
 * ReportManager - prints reports and exports to CSV only when requested.
 * Works with SimpleList + Node (no Java Collections).
 */
public class ReportManager {

    /**
     * Show pending tickets grouped by priority.
    ...
    /**
     * Show Top-K by number of notes and optionally export CSV.
    ...
    public void showTopKByNotes(SimpleList<Ticket> tickets, int k, boolean exportCsv, String csvPath) {
        ...
        // selection sort descending by note count
        for (int i = 0; i < n - 1; i++) {
            int maxIdx = i;
            for (int j = i + 1; j < n; j++) {
